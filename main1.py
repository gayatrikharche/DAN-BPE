...
from DANmodels1 import SentimentDatasetDAN, DAN
...
def main():
    ...
    if args.model == "BOW":
        ...
        # Load word embeddings (you may need to adjust how you load them)
       
        word_embeddings = read_word_embeddings("data/glove.6B.300d-relativized.txt") 

        # Prepare the dataset for DAN
        train_data = SentimentDatasetDAN("data/train.txt", word_embeddings.word_indexer)
        dev_data = SentimentDatasetDAN("data/dev.txt", word_embeddings.word_indexer)
        train_loader = DataLoader(train_data, batch_size=16, shuffle=True)
        test_loader = DataLoader(dev_data, batch_size=16, shuffle=False)
    ...
    if args.model == "BOW":
        ...
        print('\nTraining DAN model:')
        dan_model = DAN(word_embeddings=word_embeddings, hidden_size=100)  # Adjust hidden_size as needed

...
def main():
    ...
    if args.model == "BOW":
        ...
        # Prepare dataset using BPE tokenization
        train_data = SentimentDatasetDAN("data/train.txt", bpe)
        dev_data = SentimentDatasetDAN("data/dev.txt", bpe)
        train_loader = DataLoader(train_data, batch_size=16, shuffle=True)
        test_loader = DataLoader(dev_data, batch_size=16, shuffle=False)

        vocab_size = bpe.sp.get_piece_size()  # Get the vocabulary size

...
from DANmodels2 import SentimentDatasetDAN, DAN
...
def main():
    ...
    if args.model == "BOW":
        ...
        # Initialize word indexer (build vocabulary directly from the dataset)
        word_indexer = Indexer()
        train_examples = read_sentiment_examples("data/train.txt")
        dev_examples = read_sentiment_examples("data/dev.txt")
        
        # Populate word indexer
        for example in train_examples:
            for word in example.words:
                word_indexer.add_and_get_index(word)

        vocab_size = len(word_indexer)  # Get the size of the vocabulary
        embed_dim = 50
        
        # Prepare the dataset for DAN
        train_data = SentimentDatasetDAN("data/train.txt", word_indexer)
        dev_data = SentimentDatasetDAN("data/dev.txt", word_indexer)
        train_loader = DataLoader(train_data, batch_size=16, shuffle=True)
        test_loader = DataLoader(dev_data, batch_size=16, shuffle=False)
    ...
    if args.model == "BOW":
        ...
        print('\nTraining DAN model:')
        dan_model = DAN(vocab_size=vocab_size, embed_dim=embed_dim, hidden_size=100)
        dan_train_accuracy, dan_test_accuracy = experiment(dan_model, train_loader, test_loader)

        # Plotting logic for DAN can be added here as needed
        plt.figure(figsize=(8, 6))
